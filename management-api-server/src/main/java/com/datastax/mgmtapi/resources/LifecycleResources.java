...
public class LifecycleResources
{
    ...
    public synchronized Response startNode(@QueryParam("profile") String profile, @QueryParam("replace_ip") String replaceIp)
    {
        ...
        try
        {
            ...
            String cassandraLogDir = System.getenv("CASSANDRA_LOGS");
            if (cassandraLogDir == null)
            {
                // use default
                logger.debug("CASSANDRA_LOGS is not set, using default of /var/log/cassandra");
                cassandraLogDir = "/var/log/cassandra";
            }

            boolean started = false;

            if (!verifyOrCreateLogDirectory(cassandraLogDir))
            {
                logger.error(String.format(
                        "Process cannot write to %s. Please ensure that permissions are set correctly and that the CASSANDRA_LOGS environment variable is set to the desired log directory.",
                        cassandraLogDir));
            }
            else
            {
